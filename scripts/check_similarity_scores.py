...
async def main():
    ...
    for threshold in thresholds:
        results = await client.similarity_search(
            query_embedding=embedding, limit=10, similarity_threshold=threshold
        )
        ...
        if results:
            ...
            print(
                f"{threshold:<12.2f} {len(results):<10} {max_sim:<10.3f} {avg_sim:<10.3f} {min_sim:<10.3f}"
            )
        else:
    ...
    results = await client.similarity_search(
        query_embedding=embedding, limit=5, similarity_threshold=0.0
    )
