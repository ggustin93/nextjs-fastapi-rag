...
async def query_with_threshold(query: str, threshold: float) -> Dict:
    ...
    if sources:
        ...
        print(f"   ❌ No sources returned (all chunks below threshold)")
...
async def analyze_thresholds():
    ...
    for r in results:
        print(f"| {r['threshold']:.2f}     | {r['num_sources']:7d} | {r['avg_similarity']:.3f}   | {r['max_similarity']:.3f}   | {r['min_similarity']:.3f}   |")
    ...
    if optimal:
        ...
        print(f"   - Balances precision (high threshold) with recall (sources found)")
    else:
        print(f"\n⚠️  WARNING: No suitable threshold found!")
        print(f"   All thresholds either return 0 sources or very low similarity.")
        print(f"   Consider:")
        print(f"   1. Multilingual embedding model (current model is English-biased)")
        print(f"   2. Re-ingestion with better French embeddings")
        print(f"   3. Using lower threshold (0.35-0.38) as temporary fix")
    ...
    print(f"   - English-biased embeddings likely causing lower similarity scores")
    print(f"   - French technical terminology underperforms in current embedding space")
